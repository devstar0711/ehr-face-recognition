...
FFI struct ResultCompare *addFace(int32_t width,
    ...
    if (nFacesRecognized == 0) {
        ...
        dlib::matrix<float,0,1> descriptor = chips[0].face_descriptor;
        result->face_descriptor = new float[descriptor.size()];
        int i = 0;
        for (float* iterator = descriptor.begin(); iterator != descriptor.end(); iterator ++) {
            result->face_descriptor[i] = *iterator;
            i ++;
        }
    }
